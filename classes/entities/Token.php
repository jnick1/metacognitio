...
class Token
{
    ...
    //TODO: finish up any further work needed on this class. Also test it.
    ...
    private $expiration;
    /**
     * Stores a boolean indicating whether or not a token's information is stored in the database (true), or whether
     * the current Token object only exists within the current session.
     *
     * @var bool
     */
    private $inDatabase;
    ...
    public function __construct1(string $tokenID)
    {
        ...
        $params = ["s", $tokenID];
        $token = $dbc->query("select", "SELECT * FROM `token` WHERE `pkTokenID` = ? LIMIT 1", $params);
        if ($token) {
            $data = json_decode($token["blGet"], true);
